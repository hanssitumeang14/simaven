...
export function VendorSppbPage() {
  const { data: vendor } = useMyVendor()
  const [page, setPage] = useState(1)
  const { data, isLoading, isError } = useSppbList({ page, size: 20, vendor_id: vendor?.id })
      ...
      <div className="p-6">
        <Card className="p-10 text-center">
          ...
          <p className="text-sm text-muted-foreground">
            SPPB akan muncul di sini setelah RS menerbitkannya untuk pengadaan yang Anda menangkan.
          </p>
        </Card>
      </div>
    )
  ...
  return (
    <div className="space-y-4 p-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">SPPB Saya</h1>
        <p className="text-sm text-gray-600">
          Surat Pesanan Pembelian Barang untuk perusahaan Anda, beserta progres pengiriman
        </p>
      </div>
